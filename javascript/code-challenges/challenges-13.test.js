...
const longestString = (arr) => {
  return arr.indexOf(arr.reduce((acc, str) => acc.length > str.length ? acc : str, ''));
};
...
const firstLetters = (arr) => {
  return arr.map(str => str.match(/^\D/g).toString());
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => str.match(/\d/gm).join().replaceAll(',',''));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((element, index) => index % 2 === 1 ? element : '').join('');
};
