...
const screenForNames = (arr) => {
  let x = /^(M|D)(r|s)?s?\.\s\w+/;
  return arr.filter(str => x.test(str));
}
...
let biggerThanLuke = (arr) => {
  let biggerThanLukeArr = arr.filter(obj => obj.mass > 77);
  return biggerThanLukeArr.map(obj => obj.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  return arr.sort((a, b) => {
    if (typeof a[property] === 'string') {
      const strA = a[property].toUpperCase();
      const strB = b[property].toUpperCase();
      if (strA < strB) {
        return -1;
      }
      if (strA > strB) {
